...
# --- ФОНОВАЯ СЛУЖБА АВТОМАТИЧЕСКОГО ОБНОВЛЕНИЯ С GITHUB ---
async def github_auto_update_loop(bot: Bot):
    """Фоновая проверка новых коммитов на GitHub (раз в 30 секунд)."""
    ...
    while True:
        ...
        try:
            ...
            p_loc = await asyncio.create_subprocess_shell(
                "git rev-parse HEAD",
                cwd=base_dir,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
            stdout_loc, _ = await p_loc.communicate()
            local_hash = stdout_loc.decode("utf-8", errors="ignore").strip()
            if not local_hash:
                continue

            p_rem = await asyncio.create_subprocess_shell(
                "git ls-remote origin refs/heads/main",
                cwd=base_dir,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
            stdout_rem, _ = await p_rem.communicate()
            out_rem = stdout_rem.decode("utf-8", errors="ignore").strip()
            if not out_rem:
                continue

            remote_hash = out_rem.split()[0].strip()
            if remote_hash and local_hash != remote_hash:
                p_pull = await asyncio.create_subprocess_shell(
                ...
                p_log = await asyncio.create_subprocess_shell(
                    "git log -1 --pretty=%B",
                    cwd=base_dir,
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
                stdout_log, _ = await p_log.communicate()
                commit_msg = stdout_log.decode("utf-8", errors="ignore").strip() or "Обновление проекта"

                try:
                    msg_parts = [
                        "🚀 <b>АВТОМАТИЧЕСКОЕ ОБНОВЛЕНИЕ С GITHUB!</b>",
                        "━━━━━━━━━━━━━━━━━━",
                        f"📦 <b>Коммит:</b> <code>{html.escape(commit_msg[:150])}</code>",
                        f"🏷 <b>Хеш:</b> <code>{remote_hash[:7]}</code>",
                        "━━━━━━━━━━━━━━━━━━",
                        "⚡ <i>Бот автоматически перезапущен и уже работает на новой версии!</i>"
                    ]
                    msg_text = chr(10).join(msg_parts)
                    await bot.send_message(OWNER_ID, msg_text, parse_mode="HTML")
